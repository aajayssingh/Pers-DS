...
int main(){
    ...
    if ( isCrashed() ) {
        ...
        struct list *nd = (struct list*)nvmalloc(sizeof(struct list), (char*)"x");
        int node_count = 0;
        nvrecover(nd, sizeof(struct list), (char*)"x", node_count);
        printf("Recovered nd->sentinel_id = %d\n", nd->sentinel_id);
        ...
        //next node
        node_count = 1;
        nd->head = (struct node *)malloc(sizeof(struct node));
        nvrecover(nd->head, sizeof(struct node), (char*)"y", node_count);

       // node1 = (struct node *)nd->head;

        if(nd->head)
        {
           printf("GONNA ACCESS NEXT.\n");          
           printf("Recovered nd->head not null = %d\n", (struct node *)(nd->head)->id);
        }
       // getchar();
        //free(ptr);
    }
...
*/
        //preparing first node
        struct node *nd1 = (struct node *)nvmalloc(sizeof(struct node), (char *)"y");
        nd1->id = 999;
/*        nd1->c[0] = 99;
        nd1->c[1] = 100;*/
        nd1->next = NULL;
